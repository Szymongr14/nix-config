...
let
  ...
  # Every directory under ./skills/ that contains a SKILL.md is a real skill.
  # Directories starting with "." (e.g. .template) are skipped so the template
  # never gets shipped as a real skill.
...
in
{
  ...
  # Sources live under ./skills/, so a single edit updates both agents.
